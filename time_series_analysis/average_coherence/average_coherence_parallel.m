...
for ix = 1:n_mice_w_dates
  ...
  n_temp_phase_pts_vec = n_phase_pts_vec * n_tot_freqs;
  temp_phase_ixs = [ ...
    cumsum([1, n_temp_phase_pts_vec(1:(end-1) )]); ...
    cumsum(n_temp_phase_pts_vec)];

  temp_exp_phases_data = NaN(sum(n_temp_phase_pts_vec), n_region_pairs);

  parfor jx = 1:n_region_pairs
    ...
    temp_phase_region_mean = NaN(1, sum(n_temp_phase_pts_vec) );

    for px = 1:n_exp_phases
      ax = temp_phase_ixs(1,px);
      bx = temp_phase_ixs(2,px);

      temp_phase_region_mean(ax:bx) = reshape( ...
        reshape( ...
          mean( ...
            reshape(selected_data.(experiment_phases{px})', n_next_files, []), ...
            1, "omitnan"), ...
          n_tot_freqs, [])', ...
        [], 1);
    end

    temp_exp_phases_data(:,jx) = temp_phase_region_mean;
  end

  for jx = 1:n_region_pairs
    col_ax = ((jx - 1) * n_tot_freqs) + 1;
    col_bx = jx * n_tot_freqs;

    for px = 1:n_exp_phases
      row_ax = temp_phase_ixs(1,px);
      row_bx = temp_phase_ixs(2,px);

      mouse_ix_exp_phases_data.(experiment_phases{px})(:,col_ax:col_bx) = reshape( ...
        temp_exp_phases_data(row_ax:row_bx,jx), ...
        [], n_tot_freqs);
    end
  end
